...
The sheet's code
----------------
The same run also writes what Loader.gs installs and upgrades from —
edld_sheet.js and release.json — through build_bundle.py, which is where that
is described and which can be run on its own without openpyxl.
...
import build_bundle  # noqa: E402  — the sheet bundle and its manifest
from build_bundle import default_version  # noqa: E402
...
def main():
    ...
    release = build_bundle.write(args.version, out_dir)
    print(f"wrote {out_dir / build_bundle.BUNDLE_NAME} and "
          f"{build_bundle.MANIFEST_NAME} (sheet layout v{release['sheet_version']})")
