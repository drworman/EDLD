...
The sheet's code
----------------
The same run writes what Loader.gs installs and upgrades from:
    ...
    release.json    The version, the bundle's URL at this version's tag and its
                    SHA-256, the sheet layout version it brings a sheet to, and
                    the oldest loader it runs under.

The loader reads release.json from main, so a sheet's Upgrade sees a release
once main has it, and fetches the bundle from the tag the manifest names.
...
#: Raw file host for this repository. Forks change this and nothing else.
REPO_RAW = 'https://raw.githubusercontent.com/drworman/EDLD'
...
def build_bundle(version: str) -> str:
    ...
    parts = [
        '// GENERATED by sheets/build_dashboard.py from '
...
def build_release(version: str, bundle_text: str) -> dict:
    ...
    return {
        ...
        'bundle': {
            'url': f'{REPO_RAW}/{version}/sheets/edld_sheet.js',
            'sha256': hashlib.sha256(data).hexdigest(),
            'bytes': len(data),
        },
        'loader': {
            'version': _gs_int(HERE / 'Loader.gs', r'var LOADER_VERSION = (\d+);'),
            'url': f'{REPO_RAW}/{version}/sheets/Loader.gs',
        },
    }
...
def main():
    ...
    ap.add_argument('--bundle-dir', default=str(HERE),
                    help='where edld_sheet.js and release.json are written')
    args = ap.parse_args()
    ...
    import json
    out_dir = pathlib.Path(args.bundle_dir)
    bundle = build_bundle(args.version)
    (out_dir / 'edld_sheet.js').write_bytes(bundle.encode('utf-8'))
    release = build_release(args.version, bundle)
    (out_dir / 'release.json').write_text(json.dumps(release, indent=2) + '\n',
                                          encoding='utf-8')
    print(f"wrote {out_dir / 'edld_sheet.js'} and release.json "
          f"(sheet layout v{release['sheet_version']})")
