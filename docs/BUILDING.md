...
### The whole release build, locally

`scripts/build_local.sh` runs the same sequence the Release workflow runs, on
this machine: the preflight on the checkout, the PyInstaller build, both smoke
tests, the archive with the licence texts, and the SHA-256 file.

```sh
scripts/build_local.sh                 # build, test, package into out/
scripts/build_local.sh --no-package    # build and test only
scripts/build_local.sh --dir           # directory layout instead of onefile
scripts/build_local.sh --skip-tests    # skip the smoke tests
scripts/build_local.sh --sign          # also sign, using SIGNING_KEY
```

Artefacts land in `out/`, named exactly as the release ones are. On Windows run
it from Git Bash or MSYS2. PyInstaller does not cross-compile, so this builds
for the platform it runs on — three machines are still needed for a full set.

It is also the fastest way to tell a broken tree from a broken runner: if this
passes and CI does not, the difference is the runner, not the code.
...
```sh
./dist/EDLD --version     # should print the contents of ./version
...
Tag with a bare `YYYYMMDD` datestamp matching the `version` file:
