"""
components/eddn.py — EDDN (Elite Dangerous Data Network) uploader.
...
The uploader ID is sourced from journal data (state.pilot_name) — there is
no per-plugin UploaderID setting to keep in sync.
...
from core import integration_log as _ilog
from core.state import EDLD_DATA_DIR, PROGRAM, VERSION
...
class _Sender(threading.Thread):
    ...
    def __init__(self, endpoint: str, queue_file: Path) -> None:
        ...
        # Send-side stats — visible via heartbeat and on shutdown.  Letting
        # the user see "actually-sent" numbers is the only way to verify
        # EDDN uploads against a downstream consumer (eddb.io, EDSM, inara,
        # meritminer.cc, etc) that hasn't received what we think we sent.
        self._stats_lock = threading.Lock()
        self._stats: dict = {
            "sent":      0,            # successful POSTs
            "rejected":  0,            # 400/426/413 — permanent drops
            "failures":  0,            # transient failures (retried)
            "bytes_out": 0,            # gzipped POST body bytes
            "by_schema": {},           # {short_name: count}
        }
    ...
    def stats_snapshot(self) -> dict:
        with self._stats_lock:
            return {
                ...
                "by_schema": dict(self._stats["by_schema"]),
            }
    ...
    def run(self) -> None:
        # Lifecycle banner — confirms the sender thread launched.  Critical
        # because the rest of EDDN's log lines only fire on errors; without
        # this banner there's no positive evidence the sender exists.
        _ilog.sender_started("EDDN", self._queue_file)
        ...
        while not self._stop_evt.is_set():
            ...
            # Heartbeat once a minute — visible proof the thread is alive
            # plus a running tally so the user can correlate against any
            # downstream consumer that says "we haven't seen your data".
    ...
    def _send(self, msg: dict) -> tuple[bool, bool]:
        ...
        """
        # Extract a short schema label for the log line.  EDDN schemas come
        # in like "https://eddn.edcd.io/schemas/commodity/3" — we just want
        # the "commodity/3" suffix.
        schema_ref  = msg.get("$schemaRef", "")
        schema_tail = schema_ref.rsplit("/schemas/", 1)[-1] if schema_ref else "?"
        ...
        try:
            ...
            _ilog.transient("EDDN", f"POST {schema_tail} "
                                    f"({len(encoded):,} B gz, "
                                    f"{len(payload):,} B raw)")
            with urllib.request.urlopen(req, timeout=HTTP_TIMEOUT_S) as resp:
                if resp.status == 200:
                    with self._stats_lock:
                        self._stats["sent"]      += 1
                        self._stats["bytes_out"] += len(encoded)
                        self._stats["by_schema"][schema_tail] = (
                            self._stats["by_schema"].get(schema_tail, 0) + 1
                        )
                    _ilog.transient("EDDN",
                                    f"{schema_tail} accepted (HTTP 200)")
                    return (True, False)
                # Non-200 from urlopen is rare but possible (3xx etc).
                _ilog.notice("EDDN",
                             f"{schema_tail} unexpected HTTP {resp.status}")
                return (False, False)
            ...
            if e.code in (400, 426):
                ...
                _ilog.response_failed(
                    "EDDN", e.code,
                    f"{schema_tail} dropped: {body[:200]}",
                )
                return (False, True)
            if e.code == 413:
                with self._stats_lock:
                    self._stats["rejected"] += 1
                _ilog.response_failed("EDDN", 413,
                                     f"{schema_tail} payload too large — dropped")
                return (False, True)
            with self._stats_lock:
                self._stats["failures"] += 1
            _ilog.response_failed("EDDN", e.code,
                                 f"{schema_tail} — will retry")
            return (False, False)
            ...
            with self._stats_lock:
                self._stats["failures"] += 1
            _ilog.notice("EDDN",
                         f"{schema_tail} send error "
                         f"({type(exc).__name__}: {exc}) — will retry")
            return (False, False)
    ...
    def _append_disk(self, msg: dict) -> None:
        try:
            ...
            _dbg.info(f"  [EDDN] Failed to persist message to disk queue: {e}")
    ...
    def _drain_disk(self) -> None:
        ...
        try:
            ...
            _dbg.info(f"  [EDDN] Failed to update disk queue: {e}")
...
class EDDNPlugin(BasePlugin):
    ...
    SUBSCRIBED_EVENTS = [
        ...
        "Outfitting",
        "Shipyard",
        # Black market sells (emit blackmarket/1 when IllegalGoods=true)
        "MarketSell",
        # FC materials
        "FCMaterials",
    ...
    def on_load(self, core) -> None:
        ...
        # _cmdr_name is updated by the Commander journal event but for the
        # EDDN uploaderID we now prefer state.pilot_name (single source of
        # truth for commander identity across the app).  This local field
        # stays as a fallback in case state.pilot_name isn't yet populated
        # when an upload fires.
        ...
        # One-deep history of the system just left.  The game finishes writing
        # a body scan for the previous system up to a few seconds *after* the
        # FSDJump line, so an otherwise perfectly good Scan arrives while the
        # tracked location is already the new system.  Keeping the previous
        # triple lets those be augmented correctly instead of discarded.
        self._prev_system_address: int  | None = None
        self._prev_star_system:    str  | None = None
        self._prev_star_pos:       list | None = None
        ...
        self._docked_station_name: str = ""           # current docked station name
    ...
    def on_event(self, event: dict, state) -> None:
        ...
        if ev == "Docked":
            ...
            self._docked_station_name = event.get("StationName", "") or ""
        elif ev == "Undocked":
            self._docked_station_id   = None
            self._docked_station_name = ""
        ...
        match ev:
            ...
            # ── Black market sales (blackmarket/1 schema) ─────────────────────
            # MarketSell.IllegalGoods=true means this was sold at a black market.
            # Sales at regular markets don't have a corresponding EDDN schema
            # — that data is conveyed by the next Market.json sync.
            case "MarketSell" if (not state.in_preload
                                  and event.get("IllegalGoods") is True):
                self._send_blackmarket(event, is_test)
    ...
    def _uploader(self) -> str:
        # State.pilot_name is the canonical commander identity, populated by
        # the commander plugin from journal Commander/LoadGame events.  Fall
        # back to our own local _cmdr_name tracking (also fed by Commander
        # events) in case state isn't accessible, then to "unknown".
        pilot = getattr(getattr(self, "core", None), "state", None)
        if pilot is not None:
            name = getattr(pilot, "pilot_name", None)
            if name:
                return str(name)
        return self._cmdr_name or "unknown"
    ...
    def _update_location(self, event: dict) -> None:
        ...
        # Retire the current location before overwriting it, but only on a real
        # change — a second Location event for the same system must not push the
        # genuine previous system out of the slot.
        if sa is not None and self._system_address is not None and sa != self._system_address:
            self._prev_system_address = self._system_address
            self._prev_star_system    = self._star_system
            self._prev_star_pos       = list(self._star_pos) if self._star_pos else None
        if sa is not None:
    ...
    def _augment_system(self, msg: dict, require_match: bool = True) -> bool:
        ...
        # Default augmentation source is the current location.
        aug_sys, aug_pos, aug_sa = (
            self._star_system, self._star_pos, self._system_address,
        )

        if require_match and event_sa is not None and event_sa != self._system_address:
            # The game writes the last body scan of the system being left a
            # couple of seconds after the FSDJump line:
            #
            #   20:33:52  FSDJump  Colonia    SystemAddress=3238296097059
            #   20:33:54  Scan     Ogmar B    SystemAddress=84180519395914
            #
            # The scan is entirely valid, it just arrives late.  Augmenting it
            # from the tracked location would file an Ogmar body under Colonia's
            # coordinates, so dropping was the safe thing to do — but the data
            # is recoverable, because the system it belongs to is the one just
            # left and its StarPos is still known.
            if (self._prev_system_address is not None
                    and event_sa == self._prev_system_address
                    and self._prev_star_pos):
                aug_sys, aug_pos, aug_sa = (
                    self._prev_star_system,
                    self._prev_star_pos,
                    self._prev_system_address,
                )
                _dbg.info(
                    f"  [EDDN] {msg.get('event')} belongs to the previous system "
                    f"({aug_sys}) — augmenting from it rather than dropping"
                )
            else:
                _dbg.info(
        ...
        if "StarSystem" not in msg and msg.get("SystemName") is None and msg.get("System") is None:
            if not aug_sys:
                _dbg.info(f"  [EDDN] No StarSystem available for {msg.get('event')} — dropping")
                return False
            msg["StarSystem"] = aug_sys

        if "StarPos" not in msg:
            if not aug_pos:
                _dbg.info(f"  [EDDN] No StarPos available for {msg.get('event')} — dropping")
                return False
            msg["StarPos"] = list(aug_pos)

        if "SystemAddress" not in msg:
            if aug_sa is None:
                _dbg.info(f"  [EDDN] No SystemAddress available for {msg.get('event')} — dropping")
                return False
            msg["SystemAddress"] = aug_sa
    ...
    def _read_json_file(self, journal_event_name: str) -> dict | None:
        ...
        try:
            ...
            _dbg.info(f"  [EDDN] Could not read {path.name}: {e}")
            return None
    ...
    def _send_journal(self, event: dict, is_test: bool) -> None:
        ...
        # Remove internal EDLD annotation
        ...
        if "SystemAddress" not in msg:
            _dbg.info(f"  [EDDN] No SystemAddress in {ev} — dropping")
            return
    ...
    def _flush_fss_signals(self, flush_event: dict | None) -> None:
        ...
        if aug_sa is None or aug_sys is None or aug_pos is None:
            _dbg.info("  [EDDN] FSSSignalDiscovered flush: no location data — dropping batch")
            self._fss_signals = []
            return

        if self._fss_signals[0].get("SystemAddress") != aug_sa:
            _dbg.info("  [EDDN] FSSSignalDiscovered flush: first signal SystemAddress mismatch — dropping batch")
            self._fss_signals = []
            return
    ...
    def _send_codexentry(self, event: dict, is_test: bool) -> None:
        ...
        if "StarPos" not in msg:
            if not self._star_pos:
                _dbg.info("  [EDDN] CodexEntry: no StarPos available — dropping")
                return
        ...
        if not msg.get("System") or not msg.get("EntryID"):
            _dbg.info("  [EDDN] CodexEntry: missing required System or EntryID — dropping")
            return
    ...
    def _send_market_json(self, trigger: dict, is_test: bool) -> None:
        ...
        if data.get("MarketID") != trigger.get("MarketID"):
            _dbg.info("  [EDDN] Market.json MarketID mismatch — skipping")
            return
    ...
    def _send_blackmarket(self, event: dict, is_test: bool) -> None:
        """
        Emit a blackmarket/1 message from a MarketSell journal event.

        The schema renames several journal fields:
            Type         -> name
            IllegalGoods -> prohibited
            MarketID     -> marketId

        Required fields:  systemName, stationName, timestamp, name,
                          sellPrice, prohibited.

        Called only when IllegalGoods is true (regular market sales don't
        have an EDDN schema; the next Market.json sync carries that data).
        """
        name        = _canonicalise(event.get("Type", ""))
        sell_price  = event.get("SellPrice")
        system_name = self._star_system or ""
        # Station name isn't in MarketSell — pull from our tracked state.
        # When the user is docked, _docked_station_id tracks the MarketID;
        # for the station name we rely on the most recent Docked event,
        # surfaced via the parent plugin's tracking.
        station_name = self._docked_station_name or ""

        # Skip if we don't have the schema-required identifiers.  Better
        # to drop than emit a malformed message that EDDN will reject
        # with HTTP 400.
        missing = []
        if not name:         missing.append("name(Type)")
        if not system_name:  missing.append("systemName")
        if not station_name: missing.append("stationName")
        if sell_price is None: missing.append("sellPrice")
        if missing:
            _dbg.info(f"  [EDDN] blackmarket: missing {','.join(missing)} — dropping")
            return
        ...
        msg: dict = {
            ...
            "prohibited":  bool(event.get("IllegalGoods", True)),
        }
        if event.get("MarketID") is not None:
            msg["marketId"] = event.get("MarketID")

        self._add_horizons_odyssey(msg)
        self._post(_schema("blackmarket", 1, is_test), msg)
    ...
    def _send_outfitting_json(self, trigger: dict, is_test: bool) -> None:
        ...
        if data.get("MarketID") != trigger.get("MarketID"):
            _dbg.info("  [EDDN] Outfitting.json MarketID mismatch — skipping")
            return
        ...
        #
        # The set comprehension is load-bearing, not stylistic.  Outfitting.json
        # keys its Items by module id, and a station can stock the same module
        # name under more than one id (differing sku or internal id), so the
        # filtered name list can contain repeats.  outfitting/2 declares
        # "uniqueItems": true on "modules", and the EDDN gateway counts a
        # duplicate as a schema warning while still accepting the message —
        # which is why this never surfaced as an HTTP error locally.  The
        # shipyard sender already deduplicates; this one did not.
    ...
    def _send_shipyard_json(self, trigger: dict, is_test: bool) -> None:
        ...
        if data.get("MarketID") != trigger.get("MarketID"):
            _dbg.info("  [EDDN] Shipyard.json MarketID mismatch — skipping")
            return
    ...
    def _send_fcmaterials(self, event: dict, is_test: bool) -> None:
        ...
        for field in ("MarketID", "CarrierName", "CarrierID", "Items"):
            if not data.get(field) and data.get(field) != 0:
                _dbg.info(f"  [EDDN] FCMaterials.json missing {field!r} — skipping")
                return
