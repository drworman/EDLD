...
class _Sender(threading.Thread):
    ...
    def __init__(self, cmdr_provider, api_key: str, queue_file) -> None:
        ...
        self._last_send     = 0.0
        # Event names Inara has rejected as unknown.  Populated from the
        # per-event response array; an unknown name is permanent for the life
        # of the API version, so there is no point re-sending it and no point
        # logging the same rejection on every occurrence.  Session-scoped
        # deliberately: if Inara adds an event name, a restart picks it up.
        self._unknown_events: set[str] = set()

    def push(self, inara_event: dict) -> None:
        name = inara_event.get("eventName") if isinstance(inara_event, dict) else None
        if name and name in self._unknown_events:
            return
    ...
    def run(self) -> None:
        _ilog.sender_started("Inara", self._queue_file)
    ...
    def _send_batch(self, events: list[dict]) -> None:
        ...
        try:
            ...
            _ilog.request_posted("Inara", len(events), cmdr=cmdr)
            with urllib.request.urlopen(req, timeout=HTTP_TIMEOUT_S) as resp:
                self._last_send = time.monotonic()
                body = resp.read().decode("utf-8")
                result = json.loads(body) if body.strip() else {}
                header = result.get("header", {}) if isinstance(result, dict) else {}
                header_status = header.get("eventStatus", 200)
                if header_status in (200, 204):
                    _ilog.response_ok(
                        "Inara", resp.status, len(events),
                        header_status=header_status,
                    )
                    # Per-event statuses — the source of the "Errors" /
                    # "Soft errors" / "Warnings" counts shown on the
                    # Inara dashboard.  Each non-OK status is logged
                    # with its event name + Inara's status text so the
                    # user can correlate dashboard counts to real causes.
                    if isinstance(result, dict):
                        unknown = _ilog.parse_inara_events(
                            "Inara", events, result.get("events", []),
                        ) or set()
                        new_unknown = unknown - self._unknown_events
                        if new_unknown:
                            self._unknown_events |= new_unknown
                            for name in sorted(new_unknown):
                                _dbg.info(
                                    f"  [Inara] suppressing further "
                                    f"{name!r} events — Inara does not "
                                    f"recognise this event name"
                                )
                else:
                    msg = header.get("eventStatusText", "")
                    _ilog.response_failed(
                        "Inara", resp.status,
                        f"header_status={header_status}: {msg}",
                        len(events),
                    )
        ...
        match ev:
            ...
            case "Statistics":
                ...
                if wealth is not None and wealth >= 0:
                    # commanderCredits is mandatory on this Inara event; an
                    # assets-only payload is rejected outright with
                    #   setCommanderCredits status=400: No credits value provided.
                    # and the assets figure is lost with it.  Statistics does not
                    # carry a credit balance of its own, so the most recently
                    # observed one is used — in practice the LoadGame from a few
                    # seconds earlier, which is exactly the balance this wealth
                    # snapshot was taken against.
                    #
                    # Without a credits anchor there is no valid event to send,
                    # so the wealth is cached and left for the next push that
                    # does have one to carry forward via _wealth_for_credits.
                    self._last_wealth = int(wealth)
                    if self._last_credits is not None:
                        self._push(ts, "setCommanderCredits", {
                            "commanderCredits": int(self._last_credits),
                            "commanderAssets":  int(wealth),
                        })
                    else:
                        _dbg.info(
                            "  [Inara] Statistics wealth cached; no credit "
                            "balance seen yet, so it will be sent with the "
                            "next credits update."
                        )
                ...
                if modules and ship_type:
                    ...
                    for mod in modules:
                        ...
                        if not slot or not item:
                            continue
                        # Inara API field names (verified against the live Inara API)
                        entry: dict = {
                            "slotName":     slot,
                            "itemName":     item,
            ...
            case "SellOrganicData":
                # Deliberately not forwarded.  There is no Inara API event for
                # organic/exobiology sale data — the API rejected the previous
                # "addCommanderExobiology" call with
                #     400: Unknown event name 'addCommanderExobiology'.
                # on every sale.  Inara tracks exobiology progress through the
                # Exobiologist rank instead, which _push_ranks already sends via
                # setCommanderRankPilot, and the credits from the sale arrive
                # through the normal setCommanderCredits path.  Nothing is lost
                # by dropping this; re-add only if Inara publishes a real event
                # name for per-species sale data.
                pass
