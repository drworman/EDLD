...
_SPANSH_ROUTE_URLS: dict[str, list[str]] = {
    ...
    "carrier": [
        # /api/fleetcarrier/search is THE working endpoint per the
        # fc-plotbot reference implementation
        # (https://github.com/toemaus313/fc-plotbot — README explicitly
        # documents this URL).  /api/fleetcarrier/route returns HTTP 202
        # to POST but the resulting UUID never resolves anywhere, so the
        # other candidates remain only as fallbacks if /search ever stops
        # working.
