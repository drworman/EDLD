...
class _Sender(threading.Thread):
    ...
    def run(self) -> None:
        # Lifecycle banner via the unified integration_log facility — same
        # format used by Inara/EDDN/EDAstro so the file-based log reads
        # consistently across all four integrations.
        _ilog.sender_started("EDSM", self._queue_file)
        ...
        batch: list[dict] = []
        push_count     = 0   # events received from on_event since launch
        ...
        while not self._stop_evt.is_set():
            try:
                ...
                if batch and (time.monotonic() - self._last_send >= SEND_INTERVAL_S):
                    ...
                    batch = []
                # Heartbeat once a minute — surfaces "no events arriving"
                # vs "events arriving but never sending" without spamming
                # the log.  Counts include the current pending batch.
                if time.monotonic() - last_heartbeat >= 60:
                    with self._stats_lock:
                        sent = self._stats["events_sent"]
                        acc  = self._stats["events_accepted"]
                        rej  = self._stats["events_rejected"]
                        fail = self._stats["http_failures"]
                    _ilog.heartbeat(
                        "EDSM",
                        pushed=push_count, batch=len(batch),
                        sent=sent, accepted=acc, rejected=rej, failures=fail,
                    )
                    last_heartbeat = time.monotonic()
                continue
            push_count += 1
    ...
    def _send_batch(self, events: list[dict]) -> None:
        ...
        # "Missing commander name".  The endpoint expects plain form data, so
        # we send plain form data.
        ...
        # before it even examines the events.  The endpoint requires these, so
        # we send them.
        md: dict = {}
        if self._metadata_provider:
            try:
                ...
                _dbg.info(f"  [EDSM] Metadata provider error: {e}")
                md = {}
        ...
        try:
            ...
            _ilog.request_posted("EDSM", len(events),
                                 cmdr=cmdr, gameversion=md['gameversion'])
            ...
            with urllib.request.urlopen(req, timeout=HTTP_TIMEOUT_S) as resp:
                ...
                top_msgnum = (body.get("msgnum") if isinstance(body, dict)
                              else None)
                top_msg    = (body.get("msg",    "") if isinstance(body, dict)
                              else "")
                _ilog.response_ok(
                    "EDSM", resp.status, len(events),
                    msgnum=top_msgnum, msg=top_msg,
                )
            ...
            with self._stats_lock:
                self._stats["http_failures"] += 1
            _dbg.info(f"  [EDSM] HTTP {e.code} — queuing {len(events)} event(s) to disk")
            self._persist(events)
        except Exception as exc:
            with self._stats_lock:
                self._stats["http_failures"] += 1
            _dbg.info(f"  [EDSM] Send error ({type(exc).__name__}: {exc}) — queuing to disk")
            self._persist(events)
    ...
    def _inspect_response(self, body: dict, events: list[dict]) -> None:
        ...
        if isinstance(top_msgnum, int) and top_msgnum >= 200:
            ...
            if top_msgnum in (201, 202, 203, 204, 205, 206, 207, 208):
                ...
                _dbg.info(f"  [EDSM] Batch rejected ({top_msgnum}): {top_msg}")
            return
        ...
        for i, ev_resp in enumerate(per_event):
            ...
            with self._stats_lock:
                ...
                if not isinstance(msgnum, int):
                    ...
                    _ilog.event_notice("EDSM", ev_name, msgnum, msg)
                else:
                    self._stats["events_rejected"] += 1
                    bt["rejected"] += 1
                    _ilog.event_error("EDSM", ev_name, msgnum, msg)
    ...
    def _persist(self, events: list[dict]) -> None:
        try:
            ...
            _dbg.info(f"  [EDSM] Failed to persist events to disk queue: {e}")
    ...
    def _drain_disk(self) -> None:
        ...
        _dbg.info(f"  [EDSM] Replaying {len(events)} queued event(s) from disk...")
...
class EDSMPlugin(BasePlugin):
    ...
    def on_load(self, core) -> None:
        ...
        if not bool(core.cfg.app_settings.get("PrimaryInstance", True)):
            _dbg.info("  [EDSM] Uploads suppressed (PrimaryInstance = false)")
            return
        if not cfg["Enabled"]:
            return
    ...
    def print_stats(self) -> None:
        ...
        if self._sender is None:
            _dbg.info("  [EDSM] Disabled — no stats available")
            return
    ...
    def _print_stats_summary(stats: dict, label: str = "Current") -> None:
        ...
        if problems:
            _dbg.info("  [EDSM]   Per-event issues:")
    ...
    def on_event(self, event: dict, state) -> None:
        ...
        # of the most recent Fileheader/LoadGame.  Field names match the game's
        # journal naming so EDSM can read whichever variant it expects.
        ...
        if getattr(self.core, "trace_mode", False) and not getattr(state, "in_preload", False):
            _dbg.info(f"  [EDSM trace] queued {ev}")
    ...
    def _note_filtered(self, ev: str, reason: str) -> None:
        ...
        if getattr(self.core, "trace_mode", False):
            _dbg.info(f"  [EDSM trace] filtered {ev}  ({reason})")
    ...
    def _fetch_discard_list(self) -> None:
        try:
            ...
            _dbg.info(f"  [EDSM] Could not fetch discard list ({type(exc).__name__}: {exc})")
    ...
    def _fetch_position_check(self) -> None:
        ...
        try:
            ...
            _dbg.info(f"  [EDSM] Position check failed ({type(exc).__name__}: {exc})")
            return

        msgnum = body.get("msgnum")
        if isinstance(msgnum, int) and msgnum >= 200:
            _dbg.info(f"  [EDSM] Position check rejected ({msgnum}): {body.get('msg','')}")
            return
        ...
        if system:
            ...
            _dbg.info(f"  [EDSM] Last known position: {system}{extra}")
        else:
            _dbg.info(f"  [EDSM] No prior position on file — uploads will establish history.")
