"""
components/alerts.py — Combat and hazard alert tracking.
...
class AlertsPlugin(BasePlugin):
    ...
    def on_load(self, core) -> None:
        ...
        # faults: dashboard-lifetime problem reports (a component that failed
        # to load, the journal monitor dying).  Kept apart from alert_queue
        # because that deque is cleared on every context reset — LoadGame,
        # Docked, FSDJump — which during preload wipes anything raised at
        # startup before the user ever sees it.  Faults do not fade and are
        # only removed by an explicit Clear.
        self.faults: list = []
    ...
    def push_fault(self, emoji: str, text: str) -> None:
        """Record a persistent fault — a subsystem is down, not an in-game event.

        Unlike _push() this ignores in_preload and survives context resets, so
        a startup failure is still on screen when the player looks at it.
        """
        if any(f.get("text") == text for f in self.faults):
            return
        self.faults.append({"emoji": emoji, "text": text, "mono_time": None})
        gq = self.core.gui_queue
        if gq: gq.put(("alerts_update", None))

    def get_alerts(self) -> list[dict]:
        """Return current alerts list for the dashboard block renderer."""
        return list(self.faults) + list(self.alert_queue)
    ...
    def opacity_for(self, alert: dict) -> float:
        ...
        # Faults carry mono_time=None: they describe a condition that is still
        # true, not an event that happened, so they never fade.
        if alert.get("mono_time") is None:
            return 1.0
