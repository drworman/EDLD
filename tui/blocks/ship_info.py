...
class ShipInfoBlock(TuiBlock):
    ...
    def _refresh_cargo(self) -> None:
        ...
        rows.append(KVRow("Totals", cargo_totals_cols(cap_str, total)))
        ...
        if self._srv_deployed():
            ...
            if srv_items:
                rows.append(KVRow("Totals", cargo_totals_cols(tonnage, srv_total)))
            else:
                # Count-only cargo event: tonnage is known, contents are not.
                rows.append(KVRow("Carrying", cargo_totals_cols(tonnage, 0)))
