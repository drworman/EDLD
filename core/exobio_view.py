...
Turns the stored biology facts for one system (from :mod:`core.bodies_db`) plus a
...
from core.exobio_ingest import EVENTS as _EXOBIO_EVENTS
...
# Journal events after which this window must repaint, because ingesting one
# can change what build_exobio_view returns.
#
# "Scan" is in here despite carrying no biology.  This view reads the planet
# row a Scan writes: "landable" gates the genus prediction entirely, and the
# value estimate comes from atmosphere, gravity and temperature — none of which
# exist on a body known only from its FSSBodySignals stub.  Leaving it out left
# a scanned body showing the stub's blank estimate until some later bio event
# happened to repaint the window.
# tests/test_repaint_events.py checks this set against what actually changes.
REPAINT_EVENTS = frozenset(
    ("FSDJump", "Location", "CarrierJump", "Scan",
     "FSSBodySignals", "SAASignalsFound", "SAAScanComplete")
) | frozenset(_EXOBIO_EVENTS)
