...
# at the repository root. Nothing else in the tree carries a copy, so bumping a
# release is a one-line edit and drift between a constant and the file is not a
# failure mode that exists.
#
# Two layouts resolve to that same file:
#
#   source checkout   <root>/version, and this module is <root>/core/state.py,
#                     so the root is two levels up from __file__.
#   frozen binary     PyInstaller unpacks data files under sys._MEIPASS, and
#                     packaging/build_common.py places `version` at the top of
#                     the bundle, so _MEIPASS *is* the root.
#
# A missing or unreadable file degrades to "unknown" rather than raising. The
# version is cosmetic to everything except the release smoke test, which
# compares it against the tag and would catch "unknown" immediately; an
# import-time exception here would instead take the whole application down.
...
class MonitorState:
    def __init__(self):
        ...
        # Set by core.journal._monitor_died when the journal reader stops.
        # Non-empty means the dashboard is showing frozen data.
        self.monitor_error           = ""
