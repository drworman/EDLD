"""gui/blocks/exobiology.py — Exobiology window (Qt).
...
core.exobio_view.  Mirrors the Textual Exobiology block exactly.
"""
...
class ExobiologyBlock(GuiBlock):
    ...
    def refresh_data(self) -> None:
        ...
        if not view or not view["bodies"]:
            fault = explo_fault(self.core, err) if not view else ""
            self._sys_lbl.set_title(view["system"]["name"] if view else "—")
            self._sum_lbl.set_text(
                fault or "No biology here yet — surface-scan a body with signals.")
            self._scroll.set_rows([])
            return

        tot = view["totals"]
        self._sys_lbl.set_title(view["system"]["name"] or "—")
        ...
        for b in view["bodies"]:
            ...
            rows.append(self.hdr(f"{focus}{b['short'] or b['name']} · {b['bio_signals']} bio · {done}"))
