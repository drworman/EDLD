...
enabled toggle, End Session button) when the `ksw` plugin is loaded.  The
former standalone Session Management block has been folded in here so the
controls live alongside the alerts they tend to be reacting to.
"""
...
class AlertsBlock(BlockWidget):
    ...
    def build(self, parent: Gtk.Box) -> None:
        ...
        # KSW footer surface — populated on an idle callback because the
        # footer doesn't exist during build() (block_base creates it after
        # build() returns).  Same deferred pattern Commander uses for its
    ...
    def _build_ksw_footer(self) -> bool:
        ...
        footer if the ksw plugin is loaded.  Returns False so GLib only
        calls us once."""
        ft = self.footer()
        if ft is None:
            return False
        ...
        if plugin is None:
            # No ksw plugin → no footer controls.  Footer keeps only the
            # resize handle on the right.
            return False

        # "Status: ✓" / "Status: ✗" — green check when enrolled, red cross
        ...
        # kill-fire path independently checks enrollment, so flipping
        # this without enrollment is a safe no-op rather than a hazard.
        ...
        # End Session button — gated on enrollment since pressing it
        # triggers termination, which only does anything meaningful when
        # the host is enrolled.
    ...
    def _refresh_ksw_footer(self) -> None:
        ...
        if ready:
            ...
            self._ksw_status_lbl.set_tooltip_text("Machine is enrolled")
        else:
            self._ksw_status_lbl.set_markup(
                'Status: <span foreground="#cf3f3f">✗</span>'
            )
            self._ksw_status_lbl.set_tooltip_text(
                "Machine is NOT enrolled — kill-switch is inert"
            )
        # End Session button stays disabled when not enrolled (pressing it
        # does nothing useful without enrollment).
    ...
    def refresh(self) -> None:
        ...
        # Keep the KSW footer state in sync with the plugin.
        self._refresh_ksw_footer()
