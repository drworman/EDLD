"""
gui/preferences.py — Preferences window for EDLD.
...
class PreferencesWindow(Gtk.Window):
    ...
    def __init__(self, parent, core):
        super().__init__(title="EDLD Preferences")
        ...
        self._windows_dirty   = False        # Display tab: window assignment changed
        self._window_assignment: dict = {}   # working copy edited by the Display tab
    ...
    def _build_discord_tab(self, nb: Gtk.Notebook) -> None:
        ...
        for key, label, hot in [
            ("Identity",        "Use EDLD name and avatar",    False),
    ...
    # ── Display (window layout) tab ───────────────────────────────────────────

    def _available_blocks(self) -> set:
        """Windows that currently have a GTK widget.

        Exobiology is registered in the layout model but its widget doesn't
        exist yet, so it's withheld from the selector until built.
        """
        from core import layout_model as LM
        return set(LM.BLOCK_CLASS)

    def _build_display_tab(self, nb: Gtk.Notebook) -> None:
        from core import layout_model as LM
        box = self._tab(nb, "Display")

        note = Gtk.Label(
            label="Choose which window appears in each position.  Only windows of a\n"
        ...
        self._window_assignment = LM.load_assignment()
        available = self._available_blocks()
        self._win_combos: dict = {}

        last_col = None
        for info in LM.summary(self._window_assignment, available):
            if info["column"] != last_col:
                box.append(Gtk.Separator(orientation=Gtk.Orientation.HORIZONTAL))
                box.append(self._section_label(f"{info['column']} column"))
            ...
            for b in eligible:
                combo.append_text(LM.block_display(b))
            cur = info["block"]
            active = 1 + eligible.index(cur) if (cur and cur in eligible) else 0
            combo.set_active(active)
            # Connect after set_active so loading the tab doesn't mark it dirty.
            combo.connect("changed", self._on_window_combo, info["slot"], eligible)
            box.append(self._row(f"{info['slot']}  ·  {info['class_label']}", combo))
            self._win_combos[info["slot"]] = (combo, eligible)

    def _on_window_combo(self, combo, slot: str, eligible: list) -> None:
        idx = combo.get_active()
        block = None if idx <= 0 else eligible[idx - 1]
        # A window can occupy only one position — moving it clears the others.
        if block:
            for s, b in list(self._window_assignment.items()):
                if b == block and s != slot:
                    self._window_assignment[s] = None
        self._window_assignment[slot] = block
        self._windows_dirty = True
        if self._restart_banner:
            self._restart_banner.set_revealed(True)

    def _apply_windows(self) -> bool:
        """Persist the window assignment if it changed.  Returns True if saved."""
        if not getattr(self, "_windows_dirty", False):
            return False
        from core import layout_model as LM
        LM.save_assignment(self._window_assignment)
    ...
    def _on_apply(self, *_) -> None:
        ...
        needs_restart = bool(self._restart) or windows_changed
        ...
        if needs_restart:
            ...
            # launch_argv[0] is the script path (e.g. edld.py); must be kept
            # as the first argument after the interpreter, not dropped.
            # Correct form: execv(python, [python, edld.py, -g, -p, EDP1, ...])
