...
──────────────────────
  ...
  the file is discarded and the model-derived default layout is applied and saved.
...
LAYOUT_VERSION = 3          # bump when COLS/ROW_PX change or the default geometry changes
COLS     = 32
ROW_PX   = 10
GAP      = 2
MIN_W    = 3
...
def _default_layout() -> dict[str, dict]:
    """Default block geometry, derived from the shared layout model.

    Returns ``{block_name: {col, row, width, height}}`` for the windows in the
    current position assignment, so the GTK dashboard, the TUI, and the
    Preferences > Display tab all agree on where each window goes.  Read fresh
    when a grid is built so a changed assignment takes effect on next build.
    """
    from core import layout_model
    return layout_model.geometry(layout_model.load_assignment())
...
class BlockGrid:
    ...
    def _load(self) -> None:
        ...
        case we discard the file and apply the default layout so the dashboard
        looks correct immediately; the new layout is then saved so future
        launches load cleanly.

        After loading, any block present in the default layout but absent from
        ...
        try:
            ...
            # Backfill any blocks that exist in the default layout but are missing
            # from the saved file — happens when a new block is introduced.
            added = False
            for name, d in self._default.items():
